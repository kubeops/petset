...
func TestSortingActivePodsWithRanks(t *testing.T) {
	...
	for i, test := range equalityTests {
		t.Run(fmt.Sprintf("Equality tests %d", i), func(t *testing.T) {
			featuregatetesting.SetFeatureGateDuringTest(t, features.DefaultFeatureGate, features.LogarithmicScaleDown, !test.disableLogarithmicScaleDown)
			if test.p2 == nil {
				test.p2 = test.p1
			}
	...
	for i, test := range inequalityTests {
		t.Run(fmt.Sprintf("Inequality tests %d", i), func(t *testing.T) {
			featuregatetesting.SetFeatureGateDuringTest(t, features.DefaultFeatureGate, features.PodDeletionCost, !test.disablePodDeletioncost)
			featuregatetesting.SetFeatureGateDuringTest(t, features.DefaultFeatureGate, features.LogarithmicScaleDown, !test.disableLogarithmicScaleDown)
