...
// newTestPetSetPod is a test helper that builds a Pod for the given PetSet and
// ordinal without any PlacementPolicy. It mirrors the upstream
// newStatefulSetPod(set, ordinal) helper, adapting to the forked newPetSetPod
// signature which also takes a PlacementPolicy, a PodList and returns an error.
func newTestPetSetPod(set *api.PetSet, ordinal int) *v1.Pod {
	pod, err := newPetSetPod(set, nil, ordinal, nil)
	if err != nil {
		panic(err)
	}
	return pod
...
// ascendingOrdinal sorts a list of Pods by their ordinal index, ascending.
