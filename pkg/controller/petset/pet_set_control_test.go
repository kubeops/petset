...
func setupController(client clientset.Interface, apiclient versioned.Interface) (*fakeObjectManager, *fakeStatefulSetStatusUpdater, PetSetControlInterface) {
	...
	if sets, err := apiclient.AppsV1().PetSets("").List(context.TODO(), metav1.ListOptions{}); err != nil {
		panic(err)
	} else {
		for i := range sets.Items {
			if err := om.setsIndexer.Update(&sets.Items[i]); err != nil {
				panic(err)
			}
		}
	}
...
func runTestOverPVCRetentionPolicies(t *testing.T, testName string, testFn func(*testing.T, *apps.StatefulSetPersistentVolumeClaimRetentionPolicy)) {
	...
	t.Run(subtestName, func(t *testing.T) {
		featuregatetesting.SetFeatureGateDuringTest(t, features.DefaultFeatureGate, features.PetSetAutoDeletePVC, false)
...
func TestPetSetControlWithStartOrdinal(t *testing.T) {
	featuregatetesting.SetFeatureGateDuringTest(t, features.DefaultFeatureGate, features.PetSetStartOrdinal, true)
...
func TestPetSetControlRollingUpdateWithMaxUnavailable(t *testing.T) {
	featuregatetesting.SetFeatureGateDuringTest(t, features.DefaultFeatureGate, features.MaxUnavailablePetSet, true)
...
func (om *fakeObjectManager) CreatePod(ctx context.Context, pod *v1.Pod, set *api.PetSet) error {
	...
	// Mutate and store an independent copy rather than the caller's pod. In burst
	// mode the controller runs processReplica in parallel via slowStartBatch, and
	// the fork's newVersionedPetSetPod lists pods (to compute placement) while
	// other goroutines are creating pods. The caller's pod object may be aliased
	// with an entry in the shared pod cache, so writing to it here would race with
	// those concurrent ListPods reads. The real clientset likewise persists a copy
	// rather than the object handed to Create.
	cp := pod.DeepCopy()
	cp.SetUID(types.UID(cp.Name + "-uid"))
	return om.podsIndexer.Update(cp)
}

func (om *fakeObjectManager) GetPod(namespace, podName string, set *api.PetSet) (*v1.Pod, error) {
	pod, err := om.podsLister.Pods(namespace).Get(podName)
	if err != nil {
		return nil, err
	}
...
func (om *fakeObjectManager) ListPods(ns, labelSelector string, set *api.PetSet) (*v1.PodList, error) {
	pods, err := om.podsLister.Pods(ns).List(labels.Everything())
	if err != nil {
		return nil, err
	}
	podList := &v1.PodList{}
...
func (om *fakeObjectManager) GetPlacementPolicy(name string) (*api.PlacementPolicy, error) {
	return nil, nil
}

func (om *fakeObjectManager) DeletePod(pod *v1.Pod, set *api.PetSet) error {
...
func (om *fakeObjectManager) CreateClaim(claim *v1.PersistentVolumeClaim, set *api.PetSet) error {
	om.claimsIndexer.Update(claim)
	return nil
}

func (om *fakeObjectManager) GetClaim(namespace, claimName string, set *api.PetSet) (*v1.PersistentVolumeClaim, error) {
	return om.claimsLister.PersistentVolumeClaims(namespace).Get(claimName)
}

func (om *fakeObjectManager) UpdateClaim(claim *v1.PersistentVolumeClaim, set *api.PetSet) error {
	// Validate ownerRefs.
...
func (om *fakeObjectManager) addTerminatingPod(set *api.PetSet, ordinal int) ([]*v1.Pod, error) {
	pod := newTestPetSetPod(set, ordinal)
