...
import (
	...
	ocmfake "open-cluster-management.io/api/client/work/clientset/versioned/fake"
...
func TestPetSetControllerCreates(t *testing.T) {
	...
	ssc, spc, om, _ := newFakePetSetController(ctx, set)
	if err := scaleUpPetSetController(logger, set, ssc, spc, om); err != nil {
...
func TestPetSetControllerDeletes(t *testing.T) {
	...
	*set.Spec.Replicas = 0
	if err := scaleDownPetSetController(logger, set, ssc, spc, om); err != nil {
...
func TestPetSetControllerRespectsTermination(t *testing.T) {
	...
	if len(pods) != 5 {
		t.Error("PetSet does not respect termination")
	}
...
func TestPetSetControllerBlocksScaling(t *testing.T) {
	...
	*set.Spec.Replicas = 5
	...
	if len(pods) != 3 {
		t.Error("PetSet does not block scaling")
	}
	sort.Sort(ascendingOrdinal(pods))
	...
	if len(pods) != 3 {
		t.Error("PetSet does not resume when terminated Pod is removed")
	}
}

func TestPetSetControllerDeletionTimestamp(t *testing.T) {
	_, ctx := ktesting.NewTestContext(t)
	set := newPetSet(3)
	set.DeletionTimestamp = new(metav1.Time)
	ssc, _, om, _ := newFakePetSetController(ctx, set)
...
func TestPetSetControllerDeletionTimestampRace(t *testing.T) {
	_, ctx := ktesting.NewTestContext(t)
	set := newPetSet(3)
	// The bare client says it IS deleted.
	set.DeletionTimestamp = new(metav1.Time)
	ssc, _, om, ssh := newFakePetSetController(ctx, set)
...
func TestPetSetControllerAddPod(t *testing.T) {
	logger, ctx := ktesting.NewTestContext(t)
	ssc, _, om, _ := newFakePetSetController(ctx)
	set1 := newPetSet(3)
	set2 := newPetSet(3)
	pod1 := newTestPetSetPod(set1, 0)
	pod2 := newTestPetSetPod(set2, 0)
...
func TestPetSetControllerAddPodOrphan(t *testing.T) {
	logger, ctx := ktesting.NewTestContext(t)
	ssc, _, om, _ := newFakePetSetController(ctx)
	set1 := newPetSet(3)
	set2 := newPetSet(3)
...
func TestPetSetControllerAddPodNoSet(t *testing.T) {
	logger, ctx := ktesting.NewTestContext(t)
	ssc, _, _, _ := newFakePetSetController(ctx)
	set := newPetSet(3)
	...
	key, _ := ssc.queue.Get()
	if key != nil {
		t.Errorf("PetSet enqueued key for Pod with no Set %s", key)
	}
}

func TestPetSetControllerUpdatePod(t *testing.T) {
	logger, ctx := ktesting.NewTestContext(t)
	ssc, _, om, _ := newFakePetSetController(ctx)
	set1 := newPetSet(3)
	set2 := newPetSet(3)
...
func TestPetSetControllerUpdatePodWithNoSet(t *testing.T) {
	logger, ctx := ktesting.NewTestContext(t)
	ssc, _, _, _ := newFakePetSetController(ctx)
	set := newPetSet(3)
	pod := newTestPetSetPod(set, 0)
...
func TestPetSetControllerUpdatePodWithSameVersion(t *testing.T) {
	logger, ctx := ktesting.NewTestContext(t)
	ssc, _, om, _ := newFakePetSetController(ctx)
	set := newPetSet(3)
	pod := newTestPetSetPod(set, 0)
...
func TestPetSetControllerUpdatePodOrphanWithNewLabels(t *testing.T) {
	logger, ctx := ktesting.NewTestContext(t)
	ssc, _, om, _ := newFakePetSetController(ctx)
	set := newPetSet(3)
	pod := newTestPetSetPod(set, 0)
...
func TestPetSetControllerUpdatePodChangeControllerRef(t *testing.T) {
	logger, ctx := ktesting.NewTestContext(t)
	ssc, _, om, _ := newFakePetSetController(ctx)
	set := newPetSet(3)
	set2 := newPetSet(3)
...
func TestPetSetControllerUpdatePodRelease(t *testing.T) {
	logger, ctx := ktesting.NewTestContext(t)
	ssc, _, om, _ := newFakePetSetController(ctx)
	set := newPetSet(3)
	set2 := newPetSet(3)
...
func TestPetSetControllerDeletePod(t *testing.T) {
	logger, ctx := ktesting.NewTestContext(t)
	ssc, _, om, _ := newFakePetSetController(ctx)
	set1 := newPetSet(3)
	set2 := newPetSet(3)
...
func TestPetSetControllerDeletePodOrphan(t *testing.T) {
	logger, ctx := ktesting.NewTestContext(t)
	ssc, _, om, _ := newFakePetSetController(ctx)
	set1 := newPetSet(3)
	set2 := newPetSet(3)
...
func TestPetSetControllerDeletePodTombstone(t *testing.T) {
	logger, ctx := ktesting.NewTestContext(t)
	ssc, _, om, _ := newFakePetSetController(ctx)
	set := newPetSet(3)
	pod := newTestPetSetPod(set, 0)
...
func TestPetSetControllerGetPetSetsForPod(t *testing.T) {
	_, ctx := ktesting.NewTestContext(t)
	ssc, _, om, _ := newFakePetSetController(ctx)
	set1 := newPetSet(3)
	set2 := newPetSet(3)
...
func TestGetPodsForPetSetAdopt(t *testing.T) {
	set := newPetSet(5)
	pod1 := newTestPetSetPod(set, 1)
	// pod2 is an orphan with matching labels and name.
	pod2 := newTestPetSetPod(set, 2)
	pod2.OwnerReferences = nil
	// pod3 has wrong labels.
	pod3 := newTestPetSetPod(set, 3)
	pod3.OwnerReferences = nil
	pod3.Labels = nil
	// pod4 has wrong name.
	pod4 := newTestPetSetPod(set, 4)
	pod4.OwnerReferences = nil
	pod4.Name = "x" + pod4.Name

	_, ctx := ktesting.NewTestContext(t)
	ssc, _, om, _ := newFakePetSetController(ctx, set, pod1, pod2, pod3, pod4)
	...
	pods, err := ssc.getPodsForPetSet(context.TODO(), set, selector)
	if err != nil {
...
func TestAdoptOrphanRevisions(t *testing.T) {
	ss1 := newPetSetWithLabels(3, "ss1", types.UID("ss1"), map[string]string{"foo": "bar"})
	...
	_, ctx := ktesting.NewTestContext(t)
	ssc, _, om, _ := newFakePetSetController(ctx, ss1, ss1Rev1, ss1Rev2)
...
func TestGetPodsForPetSetRelease(t *testing.T) {
	_, ctx := ktesting.NewTestContext(t)
	set := newPetSet(3)
	ssc, _, om, _ := newFakePetSetController(ctx, set)
	pod1 := newTestPetSetPod(set, 1)
...
func TestOrphanedPodsWithPVCDeletePolicy(t *testing.T) {
	featuregatetesting.SetFeatureGateDuringTest(t, features.DefaultFeatureGate, features.PetSetAutoDeletePVC, true)
	...
	testFn := func(t *testing.T, scaledownPolicy, deletionPolicy apps.PersistentVolumeClaimRetentionPolicyType) {
		...
		*set.Spec.Replicas = 2
		...
		pods := []*v1.Pod{}
		pods = append(pods, newTestPetSetPod(set, 0))
		// pod1 is orphaned
		pods = append(pods, newTestPetSetPod(set, 1))
		pods[1].OwnerReferences = nil
		// pod2 is owned but has wrong name.
		pods = append(pods, newTestPetSetPod(set, 2))
		pods[2].Name = "x" + pods[2].Name

		ssc.apiClient.(*apifake.Clientset).PrependReactor("patch", "pods", func(action core.Action) (bool, runtime.Object, error) {
...
func TestStaleOwnerRefOnScaleup(t *testing.T) {
	...
	for _, policy := range []*apps.StatefulSetPersistentVolumeClaimRetentionPolicy{
		...
		ssc, spc, om, _ := newFakePetSetController(ctx, set)
		if err := scaleUpPetSetController(logger, set, ssc, spc, om); err != nil {
			t.Error(onPolicy("Failed to turn up PetSet : %s", err))
		}
		var err error
		if set, err = om.setsLister.PetSets(set.Namespace).Get(set.Name); err != nil {
		...
		*set.Spec.Replicas = 2
		if err := scaleDownPetSetController(logger, set, ssc, spc, om); err != nil {
			t.Error(onPolicy("Failed to scale down PetSet : msg, %s", err))
		}
		set, err = om.setsLister.PetSets(set.Namespace).Get(set.Name)
		if err != nil {
			t.Error(onPolicy("Could not get scaled down PetSet: %v", err))
		}
		if set.Status.Replicas != 2 {
			t.Error(onPolicy("Failed to scale petset to 2 replicas"))
		}
		...
		*set.Spec.Replicas = 3
		...
		if err := scaleUpPetSetControllerBounded(logger, set, ssc, spc, om, 10); err != nil {
			t.Error(onPolicy("Failed attempt to scale PetSet back up: %v", err))
		}
		set, err = om.setsLister.PetSets(set.Namespace).Get(set.Name)
		if err != nil {
			t.Error(onPolicy("Could not get scaled down PetSet: %v", err))
		...
		if err := scaleUpPetSetController(logger, set, ssc, spc, om); err != nil {
			t.Error(onPolicy("Failed to scale PetSet back up: %v", err))
		}
		set, err = om.setsLister.PetSets(set.Namespace).Get(set.Name)
		if err != nil {
			t.Error(onPolicy("Could not get scaled down PetSet: %v", err))
...
func newFakePetSetController(ctx context.Context, initialObjects ...runtime.Object) (*PetSetController, *StatefulPodControl, *fakeObjectManager, history.Interface) {
	// PetSet/PlacementPolicy are CRD types known only to the versioned (api) fake
	// client; the core kube fake client's scheme would panic on them. Route each
	// object to the client that understands it.
	var kubeObjects, apiObjects []runtime.Object
	for _, obj := range initialObjects {
		switch obj.(type) {
		case *api.PetSet, *api.PlacementPolicy:
	...
	apiclient := apifake.NewSimpleClientset(apiObjects...)
	...
	ocmClient := ocmfake.NewSimpleClientset()
	manifestInformerFactory := manifestinformers.NewSharedInformerFactory(ocmClient, controller.NoResyncPeriodFunc())
	ssc := NewPetSetController(
		ctx,
		informerFactory.Core().V1().Pods(),
		apiinformerFactory.Apps().V1().PetSets(),
		apiinformerFactory.Apps().V1().PlacementPolicies(),
		informerFactory.Core().V1().PersistentVolumeClaims(),
		informerFactory.Apps().V1().ControllerRevisions(),
		manifestInformerFactory.Work().V1().ManifestWorks(),
		client,
		apiclient,
		ocmClient,
	)
	...
	ssc.control = NewDefaultPetSetControl(spc, ssu, ssh, recorder)

	return ssc, spc, om, ssh
}

func fakeWorker(ssc *PetSetController) {
...
func scaleUpPetSetController(logger klog.Logger, set *api.PetSet, ssc *PetSetController, spc *StatefulPodControl, om *fakeObjectManager) error {
	return scaleUpPetSetControllerBounded(logger, set, ssc, spc, om, -1)
}

func scaleUpPetSetControllerBounded(logger klog.Logger, set *api.PetSet, ssc *PetSetController, spc *StatefulPodControl, om *fakeObjectManager, maxIterations int) error {
	om.setsIndexer.Add(set)
	ssc.enqueuePetSet(set)
	fakeWorker(ssc)
	selector, err := metav1.LabelSelectorAsSelector(set.Spec.Selector)
...
func scaleDownPetSetController(logger klog.Logger, set *api.PetSet, ssc *PetSetController, spc *StatefulPodControl, om *fakeObjectManager) error {
	selector, err := metav1.LabelSelectorAsSelector(set.Spec.Selector)
	if err != nil {
		return err
	}
