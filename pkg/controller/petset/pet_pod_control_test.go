...
// newTestStatefulPodControl adapts the upstream 4-argument NewStatefulPodControl
// call signature (client, podLister, claimLister, recorder) to the forked
// constructor which also accepts a ManifestWork client, ManifestWork lister and
// PlacementPolicy lister for distributed PetSets (all nil in these tests).
func newTestStatefulPodControl(client clientset.Interface, podLister corelisters.PodLister, claimLister corelisters.PersistentVolumeClaimLister, recorder record.EventRecorder) *StatefulPodControl {
	return NewStatefulPodControl(client, nil, podLister, nil, nil, claimLister, recorder)
}

// overlappingPetSets sorts a list of PetSets by creation timestamp, breaking
// ties by name. Mirrors the upstream overlappingStatefulSets test helper.
...
func TestStatefulPodControlCreatesPods(t *testing.T) {
	...
	control := newTestStatefulPodControl(fakeClient, nil, claimLister, recorder)
...
func TestStatefulPodControlCreatePodExists(t *testing.T) {
	...
	control := newTestStatefulPodControl(fakeClient, nil, pvcLister, recorder)
	fakeClient.AddReactor("create", "persistentvolumeclaims", func(action core.Action) (bool, runtime.Object, error) {
		create := action.(core.CreateAction)
		return true, create.GetObject(), nil
	})
...
func TestStatefulPodControlUpdateIdentityFailure(t *testing.T) {
	...
	gooPod := newTestPetSetPod(set, 0)
	gooPod.Name = "goo-0"
	podIndexer.Add(gooPod)
	podLister := corelisters.NewPodLister(podIndexer)
	claimIndexer := cache.NewIndexer(cache.MetaNamespaceKeyFunc, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc})
	claimLister := corelisters.NewPersistentVolumeClaimLister(claimIndexer)
	control := newTestStatefulPodControl(fakeClient, podLister, claimLister, recorder)
...
func TestStatefulPodControlDeletesStatefulPod(t *testing.T) {
	...
	pod := newTestPetSetPod(set, 0)
	fakeClient := &fake.Clientset{}
	control := newTestStatefulPodControl(fakeClient, nil, nil, recorder)
...
func TestStatefulPodControlClaimsMatchDeletionPolcy(t *testing.T) {
	...
	control := newTestStatefulPodControl(fakeClient, nil, claimLister, &noopRecorder{})
...
func TestPodClaimIsStale(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		claimLister := corelisters.NewPersistentVolumeClaimLister(claimIndexer)
		control := newTestStatefulPodControl(&fake.Clientset{}, nil, claimLister, &noopRecorder{})
